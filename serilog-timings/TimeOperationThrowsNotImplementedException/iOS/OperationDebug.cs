...
namespace SerilogTimings
{
	public static class Extensions
	{
		...
		public static OperationDebug BeginOperationDebug(this ILogger logger, string messageTemplate, params object[] args)
		{
			return new OperationDebug(logger, messageTemplate, args, CompletionBehaviourDebug.Abandon, LogEventLevel.Information, LogEventLevel.Warning);
		}
	...
	/// of <see cref="OperationDebug"/> are designed for use on a single thread only.
	/// </remarks>
	public class OperationDebug : IDisposable
	{
		...
		/// <summary>
		/// Property names attached to events by <see cref="OperationDebug"/>s.
		/// </summary>
		...
		CompletionBehaviourDebug _completionBehaviour;
		...
		/// <param name="args">Arguments to the log message. These will be stored and captured only when the
		/// operation completes, so do not pass arguments that are mutated during the operation.</param>
		/// <returns>An <see cref="OperationDebug"/> object.</returns>
		public static OperationDebug Begin(string messageTemplate, params object[] args)
		{
			return Log.Logger.BeginOperationDebug(messageTemplate, args);
		}
		...
		{
			if (_completionBehaviour == CompletionBehaviourDebug.Silent)
				return;
		...
		public void Dispose()
		{
			...
			{
				case CompletionBehaviourDebug.Silent:
					break;

				case CompletionBehaviourDebug.Abandon:
					Write(_target, _abandonmentLevel, OutcomeAbandoned);
					break;

				case CompletionBehaviourDebug.Complete:
					Write(_target, _completionLevel, OutcomeCompleted);
					break;
